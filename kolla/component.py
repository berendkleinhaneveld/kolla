...
class ListElementComponent(Component):
    """Custom Component class that can be instantiated for list elements
    (v-for directive)"""

    # TODO: this might work for one-level list elements, but how
    # would it work for nested lists? And how would scope be passed
    # to nested elements?
    def __init__(self, scope=None, **kwargs):
        super().__init__(**kwargs)
        # scope being a tuple with:
        # left-side (index 0): the 'lower' scope (being a tuple itself)
        # right-side (index 1): the highest scope
        self.scope = scope
        # ...FIXME
    ...
    def _lookup(self, name, context):
        ...
        def scope_lookup(self, name, context):
            return find_in_scope(name, self.scope)

        try:
            result = scope_lookup(self, name, context)
            cache[name] = scope_lookup
            return result
        except KeyError:
            super()._lookup(name, context)


def find_in_scope(key, scope: tuple[dict, tuple | None]):
    if scope and scope[0]:
        if key in scope[0]:
            return scope[0][key]
        else:
            return find_in_scope(key, scope[1])
    raise KeyError(f"{key} not in scope")
